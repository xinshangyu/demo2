...
public final class SpanUtils {
     ...
     * @return {@link SpanUtils}
     */
    public SpanUtils setForegroundColor(Context context, @ColorRes final int color) {
        this.foregroundColor = ContextCompat.getColor(context, color);
        return this;
    }
    ...
    private void updateCharCharSequence() {
        ...
        if (iconMarginGapWidth != -1) {
            if (iconMarginBitmap != null) {
                ...
                mBuilder.setSpan(new CustomIconMarginSpan(CommonBaseLibrary.getApplication(), iconMarginUri, iconMarginGapWidth, alignIconMargin), start, end, flag);
            } else if (iconMarginResourceId != -1) {
                mBuilder.setSpan(new CustomIconMarginSpan(CommonBaseLibrary.getApplication(), iconMarginResourceId, iconMarginGapWidth, alignIconMargin), start, end, flag);
            }
        }
    ...
    private void updateImage() {
        ...
        int end = start + 5;
        if (imageBitmap != null) {
            mBuilder.setSpan(new CustomImageSpan(CommonBaseLibrary.getApplication(), imageBitmap, alignImage), start, end, flag);
        } else if (imageDrawable != null) {
            mBuilder.setSpan(new CustomImageSpan(imageDrawable, alignImage), start, end, flag);
        } else if (imageUri != null) {
            mBuilder.setSpan(new CustomImageSpan(CommonBaseLibrary.getApplication(), imageUri, alignImage), start, end, flag);
        } else if (imageResourceId != -1) {
            mBuilder.setSpan(new CustomImageSpan(CommonBaseLibrary.getApplication(), imageResourceId, alignImage), start, end, flag);
        }
    }
